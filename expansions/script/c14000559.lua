...
function cm.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_ONFIELD) and cm.tgfilter(chkc,tp) end
	...
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_ATOHAND,nil,1,0,LOCATION_DECK+LOCATION_REMOVED)
end
function cm.thop(e,tp,eg,ep,ev,re,r,rp)
	local tc=Duel.GetFirstTarget()
	if tc and tc:IsRelateToEffect(e) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
		local g=Duel.SelectMatchingCard(p,cm.thfilter,p,LOCATION_DECK+LOCATION_REMOVED,0,1,1,nil,tc:GetCode())
		if #g>0 then
