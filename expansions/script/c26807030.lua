...
function c26807030.filter(c,e,tp,m)
...
function c26807030.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		local mg=Duel.GetMatchingGroup(c26807030.matfilter,tp,LOCATION_DECK,0,nil)
		return Duel.IsExistingMatchingCard(c26807030.filter,tp,LOCATION_HAND,0,1,nil,e,tp,mg)
	end
...
function c26807030.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local tg=Duel.SelectMatchingCard(tp,c26807030.filter,tp,LOCATION_HAND,0,1,1,nil,e,tp,mg)
